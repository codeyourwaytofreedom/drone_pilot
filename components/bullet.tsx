...
const Bullet:NextPage<rot_mov> = ({drone_positionX, drone_positionY,drone_rotationUD}) => {
    ...
    useFrame(()=>{
        if(bullet.current && triggered){    
            const speed = 0.1; // adjust this to control the speed of movement
            const direction = new THREE.Vector3(0, 1, 0); // initial direction vector
    ...
    return ( 
        <group position={[0,0,0]} ref={bullet} scale={0.4} rotation={[0,0,-1]}>
        <mesh>
